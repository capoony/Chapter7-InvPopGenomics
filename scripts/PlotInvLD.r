...
load_and_preprocess_data <- function(data_matrix_path, weights_matrix_path, dependent_variable_path) {
    ...
    data_matrix <- read.table(gzfile(data_matrix_path),
        header = TRUE,
        comment.char = ""
    )
    weights_matrix <- read.table(gzfile(weights_matrix_path),
        header = TRUE,
        comment.char = ""
    )
    dependent_variable <- read.table(dependent_variable_path,
        header = TRUE
    )
    ...
    labels <- colnames(data_matrix)[-c(1, 2)] # Exclude the first two columns
...
p_values_df.Eur$Continent <- rep("Europe", nrow(p_values_df.Eur))
p_values_df.NA$Continent <- rep("North America", nrow(p_values_df.NA))
...
rect_data <- data.frame(
    xmin = Start / 1000000, xmax = End / 1000000, ymin = 0, ymax = Inf,
    Chrom = Chr
)

## create Manhattan plot
PLOT <- ggplot(p_values_df, aes(x = Pos / 1000000, y = -log10(p_value))) +
    geom_point(col = rgb(0, 0, 0, 0.5), pch = 16) +
    facet_grid(Continent ~ Chrom,
        scales = "free",
        space = "free_x"
    ) +
    geom_rect(
