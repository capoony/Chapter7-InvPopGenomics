...
```
The example shown in Figure 4 below shows the distributuion of inversion-specific alleles for *In(2L)t* in a population sample collected in 2015 close to Mautern in the beautiful Wachau area along the Danube in Austria. As you can see in the scatterplot, the two set of SNPs are located around the breakpoints of *In(2L)t* and the frequencies of the invesion-specific alleles range from 0% to more than 60%, which is quite a broad range. This pattern is the prime result of sampling error in the PoolSeq data. Assume that even if a diasgnostic SNP is in full linkage disequilibrium with the inversion, it will not necessarly correctly depict the "true" frequency of the inversion in the population due to a binomial sampling error. When sequencing the pooled DNA from multiple samples, we usually aim for a sequencing depth of >50 fold, which is only a tiny fraction of the total copies of DNA from all body cells of the individuals in the extracted DNA. This sampling error leads to deviations from the expected frequency and these deviations become even bigger the lower the sequencing depths. However, if we further assume that each SNP is a reliable estimator of the "true" inversion frequency, we expect that the inferred frequencies of all marker SNPs roughly follow a binomial distribution. However, other factors, such as sequencing and mapping errors or imperfect linkage disequilibrium of some diagnostic SNPs in certain geographic regions (see above in the introduction to chapter 3) may further influence the distirbution of frequencies. Rather than using the mean frequencies across all markers, we thus use the median, which is more robust to assymetric distributions, to estimate the population inversion frequency and compare inversion patterns in all population samples. In our example in Figure 4, the median is shown as a dashed red line at app. 25%.
